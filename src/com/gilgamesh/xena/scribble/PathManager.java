...
public class PathManager {
	...
	public AbstractMap.SimpleEntry<Integer, CompoundPath> addPath(PointF point) {
		CompoundPath path = new CompoundPath(point, new CompoundPath.Callback() {
			@Override
	...
	public void removePathId(int pathId) {
		...
		for (int chunkCoordinateX
			= (int) Math.floor(
				path.bounds.left / this.CHUNK_SIZE.x); chunkCoordinateX <= (int) Math
					.floor(path.bounds.right / this.CHUNK_SIZE.x); chunkCoordinateX++) {
			for (int chunkCoordinateY
				= (int) Math.floor(
					path.bounds.top / this.CHUNK_SIZE.y); chunkCoordinateY <= (int) Math
						.floor(
							path.bounds.bottom / this.CHUNK_SIZE.y); chunkCoordinateY++) {
				Chunk chunk = chunks.get(new Point(chunkCoordinateX, chunkCoordinateY));
				if (chunk != null && chunk.getPathIds().contains(pathId)) {
					chunk.removePath(path);
					XenaApplication
						.log("PathManager::prepareEntryForRemove: Removed path " + pathId
	...
	public ArrayList<Chunk> getVisibleChunks() {
		...
		for (int i = -1; i < Math.ceil(
			1 / PathManager.CHUNK_SIZE_SCALE / PathManager.ZOOM_STEPS[this.zoomStepId
				+ PathManager.ZOOM_STEP_ID_DEFAULT]); i++) {
			for (int j = -1; j < Math.ceil(1 / PathManager.CHUNK_SIZE_SCALE
				/ PathManager.ZOOM_STEPS[this.zoomStepId
					+ PathManager.ZOOM_STEP_ID_DEFAULT]); j++) {
				Point chunkCoordinate
					= new Point(this.currentChunk.x + i, this.currentChunk.y + j);
				Chunk chunk = chunks.get(chunkCoordinate);
				if (chunk == null) {
					chunk
						= new Chunk(this, this.CHUNK_SIZE.x, this.CHUNK_SIZE.y,
							chunkCoordinate.x * this.CHUNK_SIZE.x,
